...
Call log:
  ...
  - waiting for locator('#confirm-demo')
    10 × locator resolved to <p class="mt-20" id="confirm-demo"></p>
       - unexpected value ""
...
```ts
  ...
  6  |     
  7  |     page.on("dialog",async (alert)=>{
  8  |         console.log(alert.message());
  9  |         await alert.dismiss();
  10 |     })
  11 |     
  12 |     await page.locator("(//button[@type='button'])[2]").click();
> 13 |     await expect(page.locator('#confirm-demo')).toHaveText("You pressed Cancel!");
     |                                                 ^ Error: expect(locator).toHaveText(expected) failed
  14 | 
  15 | });
  16 | 
