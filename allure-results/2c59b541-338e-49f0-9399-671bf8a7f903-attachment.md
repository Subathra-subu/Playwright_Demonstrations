...
```ts
  ...
  11 |     // page.locator('#sum1').pressSequentially(number1);
  12 |     // await page.locator('#sum2').pressSequentially(number2);
  13 | 
  14 |     const box = page.getByRole('button',{name:'Get Sum'});
  15 | 
  16 |     console.log(box.boundingBox());
  17 | 
  18 |     await box.click();
  19 | 
  20 |     await page.locator('#addmessage').isVisible();
  21 | 
> 22 |     await expect(page.locator('#addmessage')).toHaveText((number1+number2).toString());
     |                                               ^ Error: expect(locator).toHaveText(expected) failed
  23 | 
  24 | })
```