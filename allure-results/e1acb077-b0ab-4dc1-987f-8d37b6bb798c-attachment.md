...
```ts
  ...
  8  |     const number2 = 28;
  9  | 
> 10 |      await page.getByLabel('Enter first value').scrollIntoViewIfNeeded();
     |                                                 ^ Error: locator.scrollIntoViewIfNeeded: Test timeout of 30000ms exceeded.
  11 |     await page.getByLabel('Enter first value').fill(number1.toString());
  12 |     await page.getByLabel('Enter second value').fill(number2.toString());
  13 | 
