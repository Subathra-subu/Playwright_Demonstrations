...
```ts
  ...
  10 | 
  11 |     await page.getByPlaceholder('Please enter your Message').click();
  12 | 
  13 |     await page.getByPlaceholder('Please enter your Message').fill('Subathra')
  14 | 
  15 |     await page.locator('//button[@id="showInput"]').click();
  16 | 
  17 |     await page.locator('#message').isVisible();
  18 | 
> 19 |     await expect (page.locator('#message')).toHaveText("Subathra");
     |                                             ^ Error: expect(locator).toHaveText(expected) failed
  20 | 
  21 |     console.log("After message"+await page.getByPlaceholder('Please enter your Message').textContent());
  22 | });
  23 | 
```