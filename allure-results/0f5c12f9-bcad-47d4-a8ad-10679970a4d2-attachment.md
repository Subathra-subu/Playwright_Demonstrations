...
Call log:
  ...
  - waiting for locator('#addmessage')
    11 × locator resolved to <p class="mt-20" id="addmessage"></p>
       - unexpected value ""
...
- main:
  ...
  - textbox "Please enter second value": "28"
  - button "Get Sum"
  - text: "Result:"
  - paragraph
- contentinfo:
...
```ts
  ...
  11 |     await page.locator('#sum1').click();
  12 |     await page.locator('#sum1').fill(number1.toString());
  13 |     console.log("Input1:"+page.locator('#sum1').inputValue());
  14 |     await page.locator('#sum2').fill(number2.toString());
  15 |     console.log("Input2:"+page.locator('#sum2').inputValue());
  16 | 
  17 |     console.log(await page.getByRole('button',{name:'Get Sum'}).boundingBox());
  18 | 
  19 |     await page.getByRole('button',{name:'Get Sum'}).click();
  20 | 
  21 |     await page.locator('#addmessage').isVisible();
  22 | 
> 23 |     await expect(page.locator('#addmessage')).toHaveText((number1+number2).toString());
     |                                               ^ Error: expect(locator).toHaveText(expected) failed
  24 | 
  25 | })
```