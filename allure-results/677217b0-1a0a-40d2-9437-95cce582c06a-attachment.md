...
```ts
  ...
  13 |     await page.locator('#sum2').fill(number2.toString());
  14 | 
  15 |     console.log(await page.getByRole('button',{name:'Get Sum'}).boundingBox());
  16 | 
  17 |     await page.locator('#addmessage').isVisible();
  18 | 
> 19 |     await expect(page.locator('#addmessage')).toHaveText((number1+number2).toString());
     |                                               ^ Error: expect(locator).toHaveText(expected) failed
  20 | 
  21 | })
```